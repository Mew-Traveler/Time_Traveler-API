...
class TimeTravelerAPI < Sinatra::Base 
  ...
  get "/#{API_VER}/addtarget/getFlightData/:project_day/:originPlace/:destinationPlace/?" do
    ...
    result = GetFlights.call(params)
    if result.success?
      FlightRepresenter.new(result.value).to_json
    else
      FlightRepresenter.new(result.value).to_status_response
    end
  end
  ...
  post "/#{API_VER}/addtarget/addSite2db/?" do
    result = AddSite2DB.call(request.body.read)

    if result.success?
      AddSiteRepresenter.new(result.value).to_json
    else
      AddSiteRepresenter.new(result.value).to_status_response
    end
  end
